    ...
    try:
        ...
        with torch.no_grad():
            if hasattr(model, 'rel_embed') and hasattr(model, 'cross_attn') and hasattr(model, 'projection'):
                ...
                print(f"  Using full quantum-inspired feature representation")
            else:
                q_feats = torch.abs(emb1[:, :8] - emb2[:, :8]).cpu().numpy()
                print(f"  [WARNING] Model missing expected attributes, using first 8 dimensions of raw embeddings")
    except Exception as e:
