...
NOTE: This version has been corrected to compare against an independent baseline
rather than a component of the same model.
"""
...
def run_statistical_tests():
    ...
    try:
        ...
        print(f"  [ERROR] Failed to load our model: {e}")
        return {"status": "ERROR", "message": f"Failed to load our model: {e}"}
    ...
    try:
        ...
        return {"status": "ERROR", "message": f"Failed to load FIW dataset: {e}"}

    emb1, emb2, y_true_t, rels = fiw_t
    y_true = y_true_t.view(-1).numpy()
    print(f"  Loaded FIW evaluation set: {len(y_true)} pairs")

    # Get predictions from our method
    with torch.no_grad():
        meta_preds = full_meta(emb1.to(device), emb2.to(device), rels.to(device)).cpu().view(-1).numpy()

    # Get predictions from independent baseline: Cosine similarity on FIW embeddings
    # This represents a simple baseline that doesn't use our quantum-inspired architecture
    print(f"  Computing independent baseline (cosine similarity on FIW embeddings)...")
    try:
        # Convert embeddings to numpy for cosine similarity computation
        emb1_np = emb1.numpy()
        emb2_np = emb2.numpy()

        # Compute cosine similarity
        dot_product = np.sum(emb1_np * emb2_np, axis=1)
        norm_a = np.linalg.norm(emb1_np, axis=1)
        norm_b = np.linalg.norm(emb2_np, axis=1)
        baseline_preds = dot_product / (norm_a * norm_b + 1e-8)
        print(f"  Independent baseline computed")
        baseline_used = "Cosine Similarity on FIW Embeddings"
    except Exception as e:
        print(f"  [WARNING] Baseline computation failed: {e}")
        print(f"  [WARNING] Falling back to random baseline")
        baseline_preds = np.random.rand(len(y_true))
        baseline_used = "Random (Chance)"

    # Bootstrap CIs for our method
    ci_low, ci_high = bootstrap_ci(y_true, meta_preds)

    # McNemar Test against Independent Baseline
    chi2, p_val = mcnemar_test(y_true, meta_preds, baseline_preds)

    # Paired t-test on prediction errors
    err_meta = np.abs(y_true - meta_preds)
    err_baseline = np.abs(y_true - baseline_preds)
    t_stat, t_pval = stats.ttest_rel(err_meta, err_baseline)

    results = {
        "dataset": "Families In the Wild (FIW)",
        "sample_count": len(y_true),
        "accuracy": float(accuracy_score(y_true, meta_preds >= 0.5216) * 100),
        "baseline_accuracy": float(accuracy_score(y_true, baseline_preds >= 0.5216) * 100),
    ...
    print(f"Our Method Accuracy     : {results['accuracy']:.2f}%")
    print(f"Baseline Accuracy       : {results['baseline_accuracy']:.2f}% ({results['baseline_used']})")
    ...
    if p_val >= 0.05:
        print(f"\nNOTE: The difference between our method and the baseline is not statistically significant.")
        print(f"This suggests our method may not be performing better than {results['baseline_used']} on this dataset.")
    else:
        improvement = results['accuracy'] - results['baseline_accuracy']
        print(f"\nOur method shows {improvement:.2f}% absolute improvement over {results['baseline_used']}.")
