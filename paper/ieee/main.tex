...
are significantly worse. Beyond these results we report where errors concentrate --- mother--son
reaches $0.712$ ROC-AUC against father--daughter's $0.993$ on FIW despite being
its largest subset, a disparity we show is induced during training rather than
inherited from the embedding --- and how the person-level gain scales, with
most of it realised by the fifth photograph. All protocol and evaluation code,
...
while mother--son reaches $0.7124$, and mother--son is simultaneously that
corpus's largest subset. Averaged over corpora the ordering is mother--son
($0.8288$), mother--daughter ($0.8973$), father--son ($0.9172$),
father--daughter ($0.9312$).

Figure~\ref{fig:relgap} locates the origin of this disparity. On raw
embeddings with no training, the four relations span only $0.7065$--$0.7841$,
a range of $0.078$; after training the range is $0.281$. The frozen embedding
is therefore not the binding constraint, and most of the disparity is induced
during training --- plausibly because the relation-conditioned head fits the
easier relations at the expense of the hardest. We verified this is not a
labelling artefact: gender fields in the FIW metadata are complete and
well-formed across the families we audited.
