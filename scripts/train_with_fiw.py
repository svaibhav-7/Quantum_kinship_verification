...
def main():
    ...
    for existing_cache_name in ["fiw_retrain_emb_cache.pkl", "fiw_emb_cache.pkl", "embeddings_cache.pkl"]:
        existing_cache_path = os.path.join(weights_dir, existing_cache_name)
        if os.path.exists(existing_cache_path):
            try:
                ...
                cache.update(loaded)
                print(f"  Loaded {len(loaded)} embeddings from {existing_cache_name}")
    ...
    ensemble.eval()
    print(f"  Ensemble created with {len(sub_models)} sub-models")

    # Evaluate on held-out FIW test
    print(f"\n[5/6] Evaluating Ensemble on UNSEEN FIW Test ({test_emb1.shape[0]} pairs)...")

    with torch.no_grad():
        ensemble_scores = ensemble(test_emb1, test_emb2, test_rel)
